...
fetch_github_times() {
    ...
    if [ -n "$local_ver" ] && [ -n "$release_version" ] && [ "$local_ver" != "$release_version" ]; then
        ...
        if [ -n "$commit_sha" ]; then
            echo -e "${BOLD}更新详情: ${BLUE}${DMR_GITHUB_BASE}/commit/${commit_sha}${NC}"
        else
            echo -e "${BOLD}更新详情 (分支): ${BLUE}${DMR_GITHUB_BASE}/commits/${GITHUB_BRANCH}${NC}"
        fi
...
install_js_engine() {
    ...
    if ! command -v node &>/dev/null; then
        ...
        if command -v npm &>/dev/null; then
            ...
            echo -e "${BLUE}${BOLD}[INFO]${NC}${NORMAL} ${YELLOW}检测到 Node.js 已安装，但缺少 npm。${NC}"
            echo -e "${BLUE}${BOLD}[INFO]${NC}${NORMAL} 尝试通过 Corepack 启用 npm..."
            corepack enable || {
                echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} 启用 npm 失败！请手动安装。${NC}"
                return 1
            }
            echo -e "${BLUE}${BOLD}[INFO]${NC}${NORMAL} 启用完成，当前 npm 版本：$(npm -v)"
        fi
    fi
...
main_menu() {
    ...
    while true; do
        ...
        echo -e "${BLUE}${BOLD}7.${NC} ${CYAN}${BOLD}biliup-rs上传菜单${NC}"
        ...
        case $choice in
            ...
            12) require_installed && update_script ;;
             0) exit 0 ;;
            *) echo -e "${RED}无效选项 '$choice'！请输入[0-12]${NC}" ;;
        esac
