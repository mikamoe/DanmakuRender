...
check_biliup_update() {
    ...
    # 读取本地 biliup 版本输出示例: "biliup-cli 0.2.2"
    ...
    if [ "$local_version_number" != "$sanitized_latest_version" ]; then
        ...
        echo -e "${YELLOW}${BOLD}版本号：${NC}${raw_tag}"
    fi
}
...
install_dmr() {
    ...
    wget -O DanmakuRender-5.zip "https://github.com/sillda76/DanmakuRender/archive/refs/heads/v5.zip" \
      || { echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}下载失败！${NC}"; return 1; }
...
stop_dmr() {
    ...
    if [ -f "$DMR_DIR/dmr.pid" ]; then
        ...
        kill "$pid" \
          && echo -e "${BLUE}${BOLD}[INFO]${NC}${NORMAL} ${GREEN}已停止进程 $pid ${NC}" \
          && rm "$DMR_DIR/dmr.pid"
    else
        pkill -f "$DMR_CMD" \
          && echo -e "${BLUE}${BOLD}[INFO]${NC}${NORMAL} ${GREEN}已停止 ${NC}"
    fi
}
...
run_test() {
    ...
    python3 dryrun.py || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}测试运行失败！${NC}"
        return 1
    }
    deactivate
...
manual_render() {
    require_installed || return 1
    cd "$DMR_DIR" || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} 进入目录失败！"
        return 1
    }
    source venv/bin/activate || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} 激活虚拟环境失败！"
    ...
    python3 render_only.py || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} 渲染失败！"
        return 1
    }
    deactivate
}
...
install_fonts() {
    ...
    sudo mkdir -p /usr/share/fonts/truetype/microsoft || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}创建字体目录失败！${NC}"
        return 1
    }
    sudo cp "$DMR_DIR/fonts/msyh.ttf" /usr/share/fonts/truetype/microsoft/ || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}复制微软雅黑字体失败！${NC}"
        return 1
    }
    ...
    fc-list | grep "Microsoft YaHei" || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}没有找到微软雅黑字体！${NC}"
        return 1
    }

    echo -e "${BLUE}${BOLD}[INFO]${NC}${NORMAL} ${BLUE}正在刷新字体缓存...${NC}"
    sudo fc-cache -fv > /dev/null 2>&1 || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}刷新字体缓存失败！${NC}"
        return 1
    }
    ...
    sudo apt install -y fonts-noto fonts-noto-extra fonts-noto-cjk fonts-symbola fonts-noto-color-emoji > /dev/null 2>&1 || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}安装Emoji字体包失败！${NC}"
        return 1
    }
...
install_alibaba_fonts() {
    ...
    sudo cp "$DMR_DIR/fonts/AlibabaPuHuiTi-3-65-Medium.ttf" /usr/share/fonts/truetype/AlibabaPuHuiTi || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}复制阿里巴巴普惠体字体失败！${NC}"
        return 1
    }
    ...
    fc-list | grep "Alibaba PuHuiTi 3.0" || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}没有找到阿里巴巴普惠体字体！${NC}"
        return 1
    }
...
biliup_upload() {
    ...
    cd "$BILIUP_DIR" || {
        echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}进入工具目录失败！${NC}"
        return 1
    }
...
biliup_append() {
    ...
    case $type_choice in
        ...
        2)
            ...
            [ ! -d "$video_dir2" ] && {
                echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}目录不存在：$video_dir2${NC}"
                return 1
            }
...
biliup_menu() {
    while true; do
        ...
        cd "$BILIUP_DIR" || {
            echo -e "${RED}${BOLD}[ERROR]${NC}${NORMAL} ${RED}无法进入工具目录${NC}"
            return 1
        }
        ./biliup -V
        echo ""
